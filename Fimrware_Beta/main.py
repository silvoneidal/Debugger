...
# Função para atualizar valor da variável no listview
def add_variable_value(index, value):
    """Adiciona ou atualiza o valor no ListView no índice especificado."""
    children = tree.get_children()
    found = False
    for child in children:
        item_index, _, _ = tree.item(child)['values']
        if int(item_index) == index:
            tree.item(child, values=(index, tree.item(child)['values'][1], value)) #Atualiza o valor
            found = True
            break
    if not found:
        tree.insert("", "end", values=(index, f"Variável_{index}", value)) #Adiciona se não encontrar
    ...
    try:
        ...
        if selected_item:
            ...
            if new_name:  # Verifica se um novo nome foi inserido
                ...
                new_values = (current_values[0], new_name, current_values[2])  # Mantém índice e valor
...
# Treeview (ListView)
tree = ttk.Treeview(frame_listview, columns=("index", "variable", "value"), show="headings")
...
frame_add_variable = ttk.Frame(root, padding="10")
frame_add_variable.grid(row=2, column=0, sticky="ew", padx=10, pady=10) # Usando grid

variable_entry = ttk.Entry(frame_add_variable, width=35)
...
edit_button = ttk.Button(frame_add_variable, text="Editar", command=edit_variable_name, width=17)
edit_button.pack(side=tk.LEFT, padx=10)

# Botão para limpar o ListView
clear_button = ttk.Button(frame_add_variable, text="Limpar", command=clear_listview, width=17)
